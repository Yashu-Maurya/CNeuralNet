...
int main() {
    ...
    for (int j = 0; j < EPOCHS; j++) {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            Matrix* upstream_grad = layer_backward(dense, loss_gradient, LEARNING_RATE);
    ...
    printf("Input: 1\n");
    printf("Predicted: %.4f\t", result);
    printf("Learned Weight: %.4f\t", dense->weights->data[0]);
