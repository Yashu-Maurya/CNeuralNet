...
bool parse_response(const String &resp, String &type, int &index, int &label,
                    String &hex_data) {
  if (!resp.startsWith("RESP:")) {
    return false;
  }
...
bool parse_hex_pixels(const String &hex, float *data, int len) {
  ...
  for (int i = 0; i < len; i++) {
    ...
    uint8_t h_val = (h_char >= 'a')   ? (h_char - 'a' + 10)
                    : (h_char >= 'A') ? (h_char - 'A' + 10)
                                      : (h_char - '0');
    uint8_t l_val = (l_char >= 'a')   ? (l_char - 'a' + 10)
                    : (l_char >= 'A') ? (l_char - 'A' + 10)
                                      : (l_char - '0');
...
bool reset_network() {
  ...
  if (network->layer_count < 6) {
    Serial.println(
        "CRITICAL ERROR: Failed to allocate all layers (Out of Memory).");
...
void run_training(int num_samples) {
  ...
  for (int i = 0; i < num_samples; i++) {
    ...
    if (!parse_response(resp_str, type, index, label, hex_data) ||
        type != "TRAIN") {
    ...
    if ((i + 1) % 10 == 0 || (i + 1) == num_samples) {
      Serial.printf(
          "[Training] Progress: %d/%d | Avg MSE: %.4f | Free Heap: %u\n", i + 1,
          num_samples, total_loss / (float)(i + 1), ESP.getFreeHeap());
    }
...
void run_testing(int num_samples) {
  ...
  Serial.printf(
      "[Testing] Completed! Accuracy: %.2f%% (%d/%d correct) in %lu ms.\n",
...
void run_show_sample(int sample_idx) {
  ...
  Serial.printf("Actual Label: %s (%s)\n",
                (label == 1) ? "HOTDOG" : "NOT-HOTDOG",
                (pred_label == label) ? "MATCH!" : "MISMATCH");
...
void load_model() {
  ...
  if (network->layer_count > 0) {
    ...
    if (first_dense_input != INPUT_SIZE || last_dense_output != OUTPUT_SIZE) {
      Serial.printf("Error: Loaded model dimensions (%d -> ... -> %d) do not "
                    "match expected (%d -> ... -> %d).\n",
                    first_dense_input, last_dense_output, INPUT_SIZE,
                    OUTPUT_SIZE);
...
void print_menu() {
  ...
  Serial.println(
      "  train <num>  - Train on <num> random images (e.g. train 100)");
  Serial.println("  test <num>   - Test overall accuracy on <num> random test "
                 "images (e.g. test 50)");
  Serial.println("  show <index> - Render image at index in ASCII, predict, "
                 "and show details");
  Serial.println(
      "  info         - Print layers architecture and ESP32 free heap memory");
...
void setup() {
  ...
  if (!LittleFS.begin(true)) {
    Serial.println(
        "Warning: LittleFS mount failed. Model save/load features disabled.");
  }
