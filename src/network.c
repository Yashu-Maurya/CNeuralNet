...
void train_network(Network *n, Matrix *input, Matrix *target,
                   float learning_rate) {
  if (n == NULL || input == NULL || target == NULL)
    return;
  ...
  for (int i = n->layer_count - 1; i >= 0; i--) {
    Matrix *next_gradient =
        layer_backward(n->layers[i], current_gradient, learning_rate);
...
int save_network(Network *n, const char *filename) {
  ...
  fwrite(&n->layer_count, sizeof(int), 1, fp);

  for (int i = 0; i < n->layer_count; i += 1) {
    save_layer(n->layers[i], fp);
  }
  fclose(fp);
  return 0;
}
...
void load_network(Network *n, const char *filename) {
  ...
  int lc;
  if (fread(&lc, sizeof(int), 1, fp) != 1) {
    fclose(fp);
    return;
  }

  // add_layer automatically reallocates the array and increments n->layer_count
  // so we just loop and add each loaded layer.
  for (int i = 0; i < lc; i++) {
    Layer *layer = load_layer(fp);
    if (layer == NULL) {
      perror("Error loading layer.");
      break;
    }
    add_layer(n, layer);
  }
